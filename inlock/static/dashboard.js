...
const labels = { rate_limit: 'Rate limit', geo: 'Limite geográfico', user_agent: 'User-agent', ip_allowlist: 'Whitelist de IP', ip_blocklist: 'Blacklist de IP', bot_score: 'Bot score', proprietary_login: 'Login proprietário' };
const icons = { rate_limit: '⏱', geo: '◎', user_agent: '⌁', ip_allowlist: '✓', ip_blocklist: '⊘', bot_score: '◈', proprietary_login: '↪' };
...
function policySummary(policy) {
  ...
  if (policy.type === 'proprietary_login') return `${c.login_url} → ${c.success_url}${c.login_mask ? ' · tela Inlock' : ''}${c.force_desktop ? ' · modo computador' : ''}`;
  return (c.networks || []).join(', ');
}

function policyFields(type) {
  if (type === 'proprietary_login') return `<div class="full proprietary-origin"><label>URL da página de login<input name="login_url" type="url" required placeholder="https://auth.empresa.local/login"></label><label class="switch-row desktop-switch"><input type="checkbox" name="force_desktop"><i></i><span><strong>Forçar modo computador</strong><small>Somente durante o login espelhado.</small></span></label></div><label class="full">URL de sucesso<input name="success_url" type="url" required placeholder="https://app.empresa.local/dashboard"><small>O Inlock acompanha redirects entre origens e libera o projeto quando o servidor de login direcionar para esta URL.</small></label><label class="switch-row full"><input type="checkbox" name="login_mask" id="login-mask-toggle"><i></i><span><strong>Usar tela de login do Inlock</strong><small>Apresenta uma máscara responsiva enquanto o proxy autentica no serviço proprietário.</small></span></label><div id="mask-selectors" class="full mask-selectors" hidden><div><label>Localizar input de login<select name="username_selector_type"><option value="id">Por ID</option><option value="name">Por name</option><option value="xpath">Por XPath</option></select></label><label>Seletor do login<input name="username_selector" placeholder="username"></label></div><div><label>Localizar input de senha<select name="password_selector_type"><option value="id">Por ID</option><option value="name">Por name</option><option value="xpath">Por XPath</option></select></label><label>Seletor da senha<input name="password_selector" placeholder="password"></label></div><div><label>Localizar botão submit<select name="submit_selector_type"><option value="id">Por ID</option><option value="name">Por name</option><option value="type">Por type</option><option value="xpath">Por XPath</option></select></label><label>Seletor do submit<input name="submit_selector" placeholder="submit"></label></div><label class="switch-row full"><input type="checkbox" name="issue_identity_token" checked><i></i><span><strong>Disponibilizar token de autenticação</strong><small>Entrega ao projeto um JWT EdDSA em cookie HttpOnly após o login.</small></span></label><div><label>Nome do cookie <span>(opcional)</span><input name="identity_cookie_name" placeholder="inlock_identity_projeto"></label><label>Validade do token (horas)<input name="identity_ttl_hours" type="number" min="1" max="168" value="8"></label></div><label class="full">Política SameSite<select name="identity_samesite"><option value="lax">Lax (recomendado)</option><option value="strict">Strict</option></select></label></div>`;
...
function openPolicyForm(project, selectedType = 'rate_limit') {
  ...
  if (selectedType === 'proprietary_login') { const toggle=document.querySelector('#login-mask-toggle'), fields=document.querySelector('#mask-selectors'); toggle.onchange=()=>{fields.hidden=!toggle.checked; fields.querySelectorAll('[name=username_selector],[name=password_selector],[name=submit_selector]').forEach(input=>input.required=toggle.checked)}; }
  document.querySelector('#policy-form').onsubmit = async event => {
    ...
    else if (selectedType === 'proprietary_login') config = {login_url:String(form.get('login_url')), success_url:String(form.get('success_url')), force_desktop:event.target.force_desktop.checked, login_mask:event.target.login_mask.checked, username_selector_type:String(form.get('username_selector_type')), username_selector:String(form.get('username_selector')||''), password_selector_type:String(form.get('password_selector_type')), password_selector:String(form.get('password_selector')||''), submit_selector_type:String(form.get('submit_selector_type')), submit_selector:String(form.get('submit_selector')||''), issue_identity_token:event.target.issue_identity_token.checked, identity_cookie_name:String(form.get('identity_cookie_name')||''), identity_ttl_seconds:Number(form.get('identity_ttl_hours')||8)*3600, identity_samesite:String(form.get('identity_samesite')||'lax')};
...
function showView(view) {
  ...
  if (view === 'reports') window.renderReports(main, state.projects);
